 ...
 * - Based on: https://anex.us/grades/drawGraph.js
 * - Which is open-sourced: https://github.com/heydabop/grades/blob/39edc8679bba14e435af1c6863541753b2488078/website/drawGraph.js
 */
export function getChartData(sections: Section[]) {
  ...
  for(let i = 0; i < expanded.length; ++i) { //add GPAs to chart data
    ...
    let students = (expanded[i]['A'] ?? 0)
      ...
      + (expanded[i]['D'] ?? 0)
      + (expanded[i]['F'] ?? 0)
      // This number is used for weighting, so these shouldn't be included
      // + (expanded[i]['W'] ?? 0)
      // + (expanded[i]['S'] ?? 0)
      // + (expanded[i]['NCR'] ?? 0);
    ...
    if(typeof studentsMap.get(`${term} ${instructor}`) === 'undefined') { //if first section prof has taught this semester
      ...
      //increment number of students taught this semester
      studentsMap.set(
        `${term} ${instructor}`,
        studentsMap.get(`${term} ${instructor}`) + students
      )
    }
