...
export const getStaticProps: GetStaticProps<InstructorProps> = async (context) => {
  ...
  const instructorData = await instructorService.GetInstructor(instructorName.toLowerCase());
  
  const instructorNameCapitalized = instructorData !== null ? `${instructorData.lastName}, ${instructorData.firstName}` : instructorName;
  const departmentText = instructorData !== null ? getDepartmentText(instructorData) : '';
