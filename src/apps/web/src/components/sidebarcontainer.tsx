...
import { useIsCondensed } from '../lib/mediaQueries'
...
export function SidebarContainer({ condensedTitle, sidebarItems, resetScrollAfterLink, showOverflowScrollers, children }: SidebarContainerProps) {
  ...
  const [_, setTOCExpanded] = useAtom(tocAtom)
                  ...
                  <FakeLink href={e.href ?? "#"} style={{ cursor: e.disabled ? 'not-allowed' : 'auto' }}>
                    <ListItemButton
                      ...
                      <ListItemText
                        ...
                        slotProps={{
                          primary: {
                            color: isSamePath(e.href, location.pathname) ? theme.palette.text.primary : theme.palette.text.secondary,
                            fontWeight: 'unset'
                          }
                        }}
                        />
