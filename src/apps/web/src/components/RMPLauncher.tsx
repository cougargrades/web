...
import { Button, Dialog, DialogActions, DialogContent, DialogTitle, Divider, IconButton, List, ListItem, ListItemButton, ListItemSecondaryAction, ListItemText, ListSubheader, Rating, Slide, Tooltip, Typography, useMediaQuery, useTheme } from '@mui/material';
import type { TransitionProps } from '@mui/material/transitions';
import CloseIcon from '@mui/icons-material/Close'
import RateReviewIcon from '@mui/icons-material/RateReview'
import ArrowForwardIcon from '@mui/icons-material/ArrowForward'
import { Temporal } from 'temporal-polyfill';
import { getRMPProfessorSearchUrl, getRMPProfessorViewableUrl, getRMPSchoolViewableUrl, UH_RMP_SCHOOL_IDS } from '@cougargrades/vendor/rmp';
...
import UH_RED_SVG from '../assets/uh_red.svg'
...
import styles from './RMPLauncher.module.scss'
...
const RMP_CACHE_LIFETIME: Temporal.Duration = Temporal.Duration.from({ days: 7 });
  ...
  function renderListItem(item: RMPRankedSearchResult, index: number, array: RMPRankedSearchResult[]) {
    return (
      <React.Fragment key={item.id}>
        <ListItemButton className={instructorCardStyles.badgeStackListItem} component="a" href={getRMPProfessorViewableUrl(item.legacyId.toString())} target="_blank" rel="noreferrer">
          <ListItemText
            primary={
              <>
