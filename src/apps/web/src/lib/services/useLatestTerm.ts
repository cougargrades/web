...
export function useLatestTerm() {
  const query = useQuery({
    ...
    select: (termCode) => ({
      latestTerm: termCode,
      latestTermFormatted: typeof termCode === 'number' ? formatTermCode(termCode) : null,
...
export function useMissingData(): Observable<MissingTerm[]> {
  ...
  if (!data) return { data: null, error, status };
  if (!data.latestTerm) return { data: null, error, status };
