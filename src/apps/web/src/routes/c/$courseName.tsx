...
export const Route = createFileRoute('/c/$courseName')({
  loader(ctx) {
    ...
    ctx.context.queryClient.ensureQueryData(queryOptions).then(data => {
      if (isNullish(data)) {
        throw notFound();
      }
    });
    return ctx.context.queryClient;
  },
  head: async (ctx) => {
    const queryClient = ctx.loaderData;
    const queryOptions = courseDataQueryOptions(ctx.params.courseName);
...
function RouteComponent() {
  ...
  const staticHTML = (
    !isNullish(data) && data.publications.length > 0
